...
class MyCityWeatherViewModel : ViewModel() {
    ...
    fun getWeatherInfo(lat: String, lon: String, weatherRepository: GetWeatherRepository) {

        progressBarLiveData.postValue(true) // PUSH data to LiveData object to show progress bar

        weatherRepository.getWeatherByLatLong(lat, lon, object :
            ...
            override fun onRequestSuccess(multipleCityWeather: MultipleCityWeather) {
                ...
                for (data in dataList) {
                    var weatherData = WeatherData(
                        ...
                        cityName = multipleCityWeather.city?.name ?: data.name
                    )
